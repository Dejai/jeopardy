...
class Jeopardy
{
	constructor()
	{
		...
		// Associate a trello card object for the game
	...
	// SECTION: Set key attributes for the game
	setTrelloCard(trelloCard){ this.TrelloCard = trelloCard; }

	// Is a game published
	isPublished(){ return this.TrelloCard?.isPublished() ?? false; }
	...
	setCategories(jsonObj)
	{
		try{
			...
			var categoriesObject = JSON.parse(jsonObj);
	...
	/* SECTION: Get certain attributes of the game */
...
class TrelloCard
{
	constructor(trelloCard, trelloLabels=[])
	{
		...
		// Keep track of special attributes
...
class RuleOption
{
	constructor(optionObj){
		this.OptionID = optionObj?.id ?? "";
		this.Label = optionObj?.label ?? "";
		this.Type = optionObj?.type ?? "";
		this.Rule = optionObj?.rule ?? "";
		this.SubRules = optionObj?.subRules ?? [];
